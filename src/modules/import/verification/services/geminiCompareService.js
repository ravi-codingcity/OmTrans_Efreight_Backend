...
You are given:
1. A CHA document titled "CHECK LIST - BILL OF ENTRY FOR HOME CONSUMPTION" (the REFERENCE).
2. One or more SYSTEM-GENERATED document(s) maintained by the importer.

TASK: Compare the CHA Checklist against the system document(s) and decide whether the
important information matches. Read everything: headers, tables, stamps, footnotes and
values embedded in paragraphs.

Compare the key customs/shipment fields wherever present, e.g.: Bill of Entry number & date,
IEC code, Importer / Consignee name & address, Supplier / Exporter name & address, Invoice
number & date, Purchase Order, Country of Origin / Consignment, Port of Loading / Discharge,
HS / CTH / Tariff codes, Description of goods, Quantity & unit, Gross / Net weight, Number of
packages, Assessable / Invoice value & currency, Exchange rate, Duty, BL / AWB / Container
numbers, Vessel / Flight, Marks & Numbers.

RULES:
- Use SEMANTIC understanding. IGNORE differences of formatting, fonts, spacing, layout, letter
  case, punctuation and trivial abbreviations. Compare the ACTUAL meaningful values.
- A field "matches" when its meaningful value is the same in both.
- Classify every material difference as one of:
  - "mismatch"            : present in both but the values differ,
  - "incorrect"           : a value appears wrong/invalid,
  - "missing_in_system"   : present in the checklist but absent from the system document(s),
  - "missing_in_checklist": present in the system document(s) but absent from the checklist,
  - "inconsistency"       : any other detected inconsistency.
...
- "score" is overall consistency from 0 to 100.
...
function normalizeResult(parsed) {
  ...
  const matchedFields = Array.isArray(parsed.matchedFields)
    ? parsed.matchedFields
        .filter((f) => f && f.field)
        .map((f) => ({ field: String(f.field), value: f.value == null ? "" : String(f.value) }))
    : [];

  const blocking = differences.some((d) => ["mismatch", "incorrect", "missing_in_system"].includes(d.type));
...
function mockResult(checklistName, systemNames) {
  return {
    ...
    matchedFields: [
      { field: "Importer Name", value: "(sample) matched" },
      { field: "Invoice Number", value: "(sample) matched" },
    ],
    differences: [
      { field: "Assessable Value", type: "mismatch", checklistValue: "(sample) 12,50,000", systemValue: "(sample) 12,05,000", detail: "Sample mismatch — enable live AI for real results." },
      { field: "HS Code", type: "missing_in_system", checklistValue: "(sample) 61103010", systemValue: null, detail: "Sample missing field — enable live AI for real results." },
    ],
    counts: { matched: 2, differences: 2, mismatched: 1, missingInSystem: 1, missingInChecklist: 0 },
